...
-- DENSE Rank assign an incrementing number to each row but if the (ordered by column) value is duplicated he give it the same
-- value and continue incrementing the last assigned value
select * ,DENSE_Rank() over (order by Students.grade desc) from Students
